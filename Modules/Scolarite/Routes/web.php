...
Route::prefix('scolarite')->group(function () {
    ...
    // Fiches de présence
    Route::get('/scolarite/fiche-presence', [InscriptionController::class, 'genererFichePresence'])->name('scolarite.fiche.presence');

    // Listes d'affichage
    Route::get('/scolarite/liste-affichage', [InscriptionController::class, 'genererListeAffichage'])->name('scolarite.liste.affichage');


    Route::get('/scolarite/fiche-presence-avancee', [InscriptionController::class, 'genererFichePresenceAvancee'])->name('scolarite.fiche.presence.avancee');
    Route::get('/scolarite/fiche-presence-pdf', [InscriptionController::class, 'genererFichePresencePdf'])->name('scolarite.fiche.presence.pdf');
    Route::get('/scolarite/fiches-pdf-par-classe', [InscriptionController::class, 'genererFichesPdfParClasse'])->name('scolarite.fiches.pdf.classe');



    // OU si vous voulez garder les deux, assurez-vous qu'elles pointent vers la même méthode
    Route::get('/scolarite/fiches-pdf', [InscriptionController::class, 'genererFichesPdfParClasse'])->name('scolarite.fiches.pdf');


    // Routes pour les nouveaux documents
    Route::get('/scolarite/registre-bibliotheque', [InscriptionController::class, 'genererRegistreBibliotheque']) ->name('scolarite.registre.bibliotheque');

    Route::get('/scolarite/fiche-dossier-candidat', [InscriptionController::class, 'genererFicheDossierCandidat'])->name('scolarite.fiche.dossier.candidat');

    Route::get('/scolarite/certificat-scolarite', [InscriptionController::class, 'genererCertificatScolarite'])->name('scolarite.certificat.scolarite');

    Route::get('/scolarite/releve-notes', [InscriptionController::class, 'genererReleveNotes'])->name('scolarite.releve.notes');

    Route::get('/scolarite/releve-notes-vide', [InscriptionController::class, 'genererReleveNotesVide'])->name('scolarite.releve.notes.vide');

    // Routes pour les relevés de notes paramétrables - SANS préfixe en double
    Route::get('/releve-notes-vide-parametres', [InscriptionController::class, 'showParametresReleveNotes'])->name('scolarite.releve.notes.vide.parametres');

    Route::get('/releve-notes-vide-generer', [InscriptionController::class, 'genererReleveNotesParametrable'])->name('scolarite.releve.notes.vide.generer');

    Route::get('/releve-notes-matiere/{classeId}/{matiere}', [InscriptionController::class, 'genererReleveNotesParMatiere'])->name('scolarite.releve.notes.matiere');

    // Routes pour le paramétrage unifié
    Route::get('/generer-documents', [InscriptionController::class, 'showGenererDocuments'])->name('scolarite.generer.documents');

    Route::get('/generer-document-pdf', [InscriptionController::class, 'genererDocumentParametrable'])->name('scolarite.generer.document.pdf');


    // Route pour générer le document (POST) - AJOUTEZ CETTE LIGNE
    Route::post('/generer-document-parametrable', [InscriptionController::class, 'genererDocumentParametrable'])->name('scolarite.generer.document.parametrable');
