    ...
    <v-card variant="outlined" style="border: 2px solid #7d002c">
        ...
        <div style="margin: 10px;font-size: 16px;">
            <v-alert v-model="alertFirst" border="start" variant="tonal" color="primary" type="info" title="Note">
                <li>
                    Cette section vous permet de configurer les <b>TYPE DE FRAIS, DOCUMENTS et NOMBRE LIMITE</b> des
                    élèves autorisés dans une seule classe
                </li>
                <li>
                    Le formulaire sera valide <strong>si et seulement si </strong>les deux tableaux ont au moins un
                    element coché
                </li>
                <li>
        ...
        <v-expansion-panels v-model="panel">
            <v-expansion-panel>
                    ...
                    <template v-slot:default="{ open }">
                        <v-row no-gutters>
                            <v-col cols="4" class="d-flex justify-start">
                                Cocher les&nbsp;<span style="font-size: 15px; color: blue;">TYPES DE
                                    FRAIS</span>&nbsp;que vous utilisiez
                            </v-col>
                ...
                <v-expansion-panel-text>
                    <v-data-table v-model="form.selected_frais" :headers="headers_frais" :items="type_frais"
                        @update:modelValue="rowClickFrais(form.selected_frais)" item-value="id" show-select
            ...
            <v-expansion-panel>
                <v-expansion-panel-title v-slot="{ open }">
                    <v-row no-gutters>
                        <v-col cols="4" class="d-flex justify-start">
                            Cocher les&nbsp;<span style="font-size: 15px; color: blue;">DOCUMENTS REQUIS</span>&nbsp;que
                            vous
                            utilisiez
                        </v-col>
                        <v-col cols="8" class="text-grey">
                            <v-fade-transition leave-absolute>
                                ...
                                <span v-else key="1">
                                    {{ items_documents.length == 0 ? 'Vous n\'avez coché aucun document' :
                                    items_documents }}
                                </span>
                            </v-fade-transition>
                        </v-col>
                ...
                <v-expansion-panel-text>
                    <v-row no-gutters>
                        <v-spacer></v-spacer>
                        <v-data-table v-model="form.selected_documents" :headers="headers_documents"
                            :items="type_documents" @update:modelValue="rowClickDocument(form.selected_documents)"
                            item-value="id" show-select class="elevation-1">
                            <template v-slot:item.obligatoire="{ item }">
                                <v-checkbox v-model="item.obligatoire"
                                    :label="`${item.obligatoire}` == 'true' ? 'Oui' : 'Non'"></v-checkbox>
                            </template>
                        </v-data-table>
                    </v-row>
                </v-expansion-panel-text>
            ...
            <v-expansion-panel>
                <v-expansion-panel-title v-slot="{ open }">
                    <v-row no-gutters>
                        <v-col cols="5" class="d-flex justify-start">
                            Définissez le&nbsp;<span style="font-size: 15px; color: blue;">NOMBRE LIMITE</span>&nbsp;des
                            élèves dans
                            une classe
                        </v-col>
                ...
                <v-expansion-panel-text>
                    <v-row justify="space-around" no-gutters>
                        <v-col cols="3">
                            <v-text-field v-model="form.nbre_limite" label="Nombre limite" type="number"></v-text-field>
                        </v-col>
                    </v-row>
                </v-expansion-panel-text>
            </v-expansion-panel>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn variant="text" color="secondary" @click="retour">
                    Annuler
                </v-btn>
