                    ...
                    <v-card-text>
                        ...
                        <v-card outlined>
                            <v-card-text id="heit">
                                <v-row dennse>
                                    ...
                                    <v-col>
                                        <VueDatePicker 
                                            ...
                                            locale="fr" cancelText="Annuler" selectText="Confirme"
                                            flow="calendar"
                                            date-picker
                                            range
                                ...
                                <br>
                                <v-expansion-panels>
                                    <v-expansion-panel id="seanceId" v-for="(day, index) in daysOfWeek" :key="index">
                                        <v-expansion-panel-title>
                                        {{ `Les seances du ${day}` }}
                                        </v-expansion-panel-title>
                                        <v-expansion-panel-text>
                                        <v-row v-for="(seance, seanceIndex) in form.seances[day]" :key="seanceIndex" dense>
                                            <v-col md="3">
                                                <VueDatePicker
                                                v-model="seance.horaire" time-picker range />
                                            </v-col>
                                            <v-col md="3">
                                                <text-field density="medium" dense label="Matiere" placeholder="Matiere" v-model="seance.name"></text-field>
                                            </v-col>
                                            <v-col md="5">
                                                <autocomplete dense label="Salle" item-title="name" item-value="id" :items="fillieres" multiple chips v-model="seance.filliere">
                                                </autocomplete>
                                            </v-col>
                                            <v-col md="1">
                                                <v-icon color="error" :disabled="!(form.seances[day].length > 1)" @click="removeRow(seance)" :icon="icon.mdiCloseCircle"></v-icon>
                                            </v-col>
                                        </v-row>
                                        <v-row dense>
                                            <v-col offset-md="11" md="1">
                                                <v-icon color="success" @click="addRow(day)" :icon="icon.mdiPlusCircle"></v-icon>
                                            </v-col>
                                        </v-row>
                                        </v-expansion-panel-text>
                                    </v-expansion-panel>
