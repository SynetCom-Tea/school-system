  ...
  <form @submit.prevent="submitForm" novalidate>
    <v-container fluid>
      <v-card variant="outlined" style="border: 2px solid #7d002c">
        <v-card-title style="color: white; background-color: #7d002c">Affectation de matière aux niveaux</v-card-title>
        <v-divider></v-divider>
        <br />
        <div style="margin: 10px">
          <v-alert v-model="alertFirst" border="start" variant="tonal" closable close-label="Close Alert"
            color="primary" type="info" title="Information">
            <li>
        ...
        <v-card>
          ...
          <v-card-text>
            <v-row v-if="section_id == 3">
              <v-col md="3"></v-col>
              <v-col md="4">
                <Autocomplete :items="filieres" variant="outlined" item-value="id" item-title="code"
                  v-model="form.filiere" @update:modelValue="submitForm(affectation)" placeholder="Cycle/Filière"
                  isRequired @change="verify(affectation)" label="Cycle/Filière" class="mt-2"
            ...
            <v-row disabled :key="affectation.id" v-for="(affectation, i) in form.Affectations">
              ...
              <v-col md="3">
                <Autocomplete :items="matieres" variant="outlined" item-value="id" item-title="nom"
                  v-model="affectation.matiere_id" @update:modelValue="submitForm(affectation)" placeholder="Matières"
                  isRequired @change="verify(affectation)" label="Matières" class="mt-2"
                  :rules="[(v) => !!v || 'Ce champ est requis!']">
                </Autocomplete>
              </v-col>
              <v-col md="3">
                <Autocomplete :items="niveaux" variant="outlined" item-value="id" item-title="code"
                  v-model="affectation.niveau_id" @update:modelValue="submitForm(affectation)" placeholder="Niveaux"
                  isRequired multiple chips @change="verify(affectation)" label="Niveaux" class="mt-2"
                  :rules="[(v) => !!v || 'Ce champ est requis!']">
                </AutoComplete>
              </v-col>
              <v-col md="2">
                <TextField type="number" min="1" label="Volume Horaire" class="mt-2" :isRequired="true"
                  placeholder="Volume Horaire" v-model="affectation.volume_horaire" @change="verify(affectation)"
                  @update:modelValue="submitForm(affectation)" :rules="[(v) => !!v || 'Ce champ est requis!']">
                </TextField>
              </v-col>
              <v-col md="2" v-if="section_id == 1">
                <TextField type="number" min="0" label="Notation" class="mt-2" :isRequired="true" placeholder="Notation"
                  v-model="affectation.coefficient" @change="verify(affectation)"
                  @update:modelValue="submitForm(affectation)" :rules="[(v) => !!v || 'Ce champ est requis!']">
                </TextField>
              </v-col>

              <v-col md="2" v-else>
                <TextField type="number" min="1" label="Coefficient" class="mt-2" :isRequired="true"
                  placeholder="Coefficient" v-model="affectation.coefficient" @change="verify(affectation)"
                  @update:modelValue="submitForm(affectation)" :rules="[(v) => !!v || 'Ce champ est requis!']">
                </TextField>
              </v-col>
              <v-col md="1">
                <br />
                <Button type="button" variant="outlined" :disabled="!(form.Affectations.length > 1)" icon
                  @click="removeRow(affectation)" size="large" small color="error">
            ...
            <v-row>
              <v-col offset-md="10" cols="4" style="height: 90px">
                <Button type="button" variant="outlined" @click="addRow" icon size="large" color="primary">
