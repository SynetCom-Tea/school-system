...
<template>
    ...
    <v-card variant="outlined" style="border: 2px solid #7d002c">
        <v-card-title style="color: white; background-color: #7d002c" v-if="systemeLMD == null">GESTION DES MATIERES PAR
            NIVEAUX</v-card-title>
        <v-card-title style="color: white; background-color: #7d002c" v-else>GESTION DES UNITES DES
            ENSEIGNEMENTS</v-card-title>
        <v-divider></v-divider>
                    ...
                    <v-card-text>
                        <v-form ref="form">
                            <v-row v-if="section_id == 3 || section_id == 4" style="height: 80px">
                                <v-col cols="6" md="6">
                                    <autocomplete :disabled="true" label="cycle/filière" chips :items="filieres"
                                        variant="outlined" itemValue="id" itemTitle="code"
                                        v-model="form.cycle_filiere_id" class="mt-1" isRequired
                                        :rules="[(v) => !!v || 'Ce champ est requis!']">
                                    </autocomplete>
                                </v-col>
                                <v-col cols="6" md="6">
                                    <autocomplete :disabled="true" label="Niveau" chips :items="niveaux"
                                        variant="outlined" itemValue="id" itemTitle="libelle" v-model="form.niveau_id"
                                        class="mt-1" isRequired :rules="[(v) => !!v || 'Ce champ est requis!']">
                                    </autocomplete>
                                </v-col>
                            </v-row>
                            ...
                            <v-row style="height: 80px">
                                <v-col cols="6" md="6" v-if="systemeLMD != null">
                                    <autocomplete :disabled="true" label="Unité d'enseignement" chips :items="ues"
                                        variant="outlined" itemValue="id" itemTitle="libelle" v-model="form.ue_id"
                                        class="mt-1" isRequired :rules="[(v) => !!v || 'Ce champ est requis!']">
                                    </autocomplete>
                                </v-col>
                                <v-col cols="6" md="6" v-if="ajout == false">
                                    <autocomplete :disabled="true" label="Matière" placeholder="Matière"
                                        :items="matieres" variant="outlined" class="mt-1" itemValue="id" itemTitle="nom"
                                        v-model="form.matiere_id" isRequired
                                        :rules="[(v) => !!v || 'Ce champ est requis!']">
                                    </autocomplete>
                                </v-col>
                                <v-col cols="6" md="6" v-if="ajout == true">
                                    <autocomplete label="Matière" :items="matieres" placeholder="Matière"
                                        variant="outlined" class="mt-1" itemValue="id" itemTitle="nom"
                                        v-model="form.matiere_id" isRequired
                                        :rules="[(v) => !!v || 'Ce champ est requis!']">
                            ...
                            <v-row style="height: 80px">
                                <v-col cols="6" md="6">
                                    <TextField class="mt-2" min="1" type="number" label="Volume_horaire"
                                        placeholder="Volume_horaire" v-model="form.volume_horaire" isRequired
                                        :rules="rules"></TextField>
                                </v-col>
                                <v-col cols="6" md="6" v-if="section_id != 1">
                                    <TextField class="mt-1" min="1" type="number" label="Coefficient"
                                        placeholder="Coefficient" v-model="form.coefficient" isRequired :rules="rules">
                                    </TextField>
                                </v-col>
                                <v-col cols="6" md="6" v-else>
                                    <TextField class="mt-1" min="0" max="20" type="number" label="Notation"
                                        placeholder="Notation" v-model="form.notation" isRequired :rules="rules">
                                    </TextField>
                                </v-col>
                            </v-row>
        ...
        <v-card-text>
            <Datatable v-if="section_id == 1" titleDatatable="Liste des matières par niveau" :headers="headerspri"
                :items="niveauMatieres" :permission="'manage_school|affectation_niveau_matiere.create'"
                ...
                <template v-slot:item.matiere="{ item, index }">
                    <v-chip-group column selected-class="text-purple">
                        <v-chip v-for="tag in item.matiere">
                            ...
                            <v-icon size="small" class="me-2" title="Modifier" @click="editItem(tag)"
                                v-permission:any="'manage_school|affectation_niveau_matiere.update'"
                                :icon="icons.mdiPencil" color="orange"></v-icon>
                            ...
                            <v-icon end size="small" class="me-2" title="Supprimer" @click="deleteItem(tag)"
                                v-permission:any="'manage_school|affectation_niveau_matiere.delete'"
                ...
                <template v-slot:item.actions="{ item }">
                    ...
                    <v-icon size="small" class="me-2" title="Supprimer" @click="deleteItemNM(item)"
                        v-permission:any="'manage_school|affectation_niveau_matiere.delete'" :icon="icons.mdiDelete"
                        color="red">
                    </v-icon>
                </template>
            </Datatable>
            <Datatable v-if="section_id == 2" titleDatatable="Liste des matières par niveau" :headers="headers"
                :permission="'manage_school|affectation_niveau_matiere.create'" :items="niveauMatieres"
                :functionOnClickAddButton="create">
                <template v-slot:item.matiere="{ item, index }">
                    <v-chip-group column selected-class="text-purple">
                        <v-chip v-for="tag in item.matiere">
                            {{ tag.matiere.nom }} =>{{ 'Coeff: ' }} {{ tag.coefficient }}
                            <v-icon end size="small" class="me-2" title="Modifier" @click="editItem(tag)"
                                v-permission:any="'manage_school|affectation_niveau_matiere.update'"
                                :icon="icons.mdiPencil" color="orange">
                            </v-icon>
                            <!-- <v-icon end color="primary" :icon="icons.mdiEye" title="Détail l'établissement" style="top: 0; left: 0; display: absolute" @click="showItem(tag)"></v-icon> -->
                            <v-icon end size="small" class="me-2" title="Supprimer" @click="deleteItem(tag)"
                                v-permission:any="'manage_school|affectation_niveau_matiere.delete'"
            ...
            <Datatable v-if="section_id >= 3 && systemeLMD != null"
                titleDatatable="Liste des unités des enseignements par niveau"
                :permission="'manage_school|affectation_niveau_matiere.create'" :headers="headersupue"
                :items="niveauMatieres" :functionOnClickAddButton="create">
                    ...
                    <v-chip-group size="small" column selected-class="text-purple">
                        <v-chip v-for="tag in item.ues" style=" height: auto;  ">
                            ...
                            <v-chip-group column selected-class="text-purple" style="width:90%;">
                                <v-chip size="small" :key="i" v-for="(t, i) in tag.matieres"
                                    style=" color: white; background-color: #7d002c; size:10px;">
                                    {{ t.matiere.nom }}
                                    {{ '/ VH: ' }}{{ t.volume_horaire }}
                                    {{ ';Coeff: ' }}{{ t.coefficient }}
                                    <v-icon end size="small" class="me-2" title="Modifier" @click="editItem(t)"
                                        v-permission:any="'manage_school|affectation_niveau_matiere.update'"
                                        :icon="icons.mdiPencil" color="orange">
                                    </v-icon>
                                    <v-icon end size="small" class="me-2" title="Supprimer" @click="deleteItem(t)"
                                        v-permission:any="'manage_school|affectation_niveau_matiere.delete'"
                                        :icon="icons.mdiCloseCircle">
                                    </v-icon>
                                </v-chip>
                            ...
                            <v-icon end size="small" class="me-2" title="Ajouter des cycles" @click="createmat(tag)"
                                v-permission:any="'manage_school|manage_school|affectation_niveau_matiere.create'"
                                :icon="icons.mdiPlusCircle" color="primary"></v-icon>
                            <!-- <v-icon end color="primary" :icon="icons.mdiEye" title="Détail l'établissement" style="top: 0; left: 0; display: absolute" @click="showItem(tag)"></v-icon> -->
                            <v-icon end size="small" class="me-2" title="Supprimer" @click="deleteItemUE(tag)"
                                v-permission:any="'manage_school|affectation_niveau_matiere.delete'"
                                :icon="icons.mdiCloseCircle">
                            </v-icon>
            ...
            <Datatable v-if="section_id >= 3 && systemeLMD == null" titleDatatable="Liste des matières par niveau"
                :headers="headersup" :items="niveauMatieres"
                :permission="'manage_school|affectation_niveau_matiere.create'" :functionOnClickAddButton="create">
                <template v-slot:item.matiere="{ item, index }">
                    <v-chip-group column selected-class="text-purple">
                        <v-chip v-for="tag in item.matiere">
                            {{ tag.matiere.nom }} =><v-chip> {{ 'VH: ' }} {{ tag.volume_horaire }}</v-chip> <v-chip> {{
                                ' Coeff: ' }} {{ tag.coefficient }}</v-chip>
                            <v-icon size="small" class="me-2" title="Modifier" @click="editItem(tag)"
                                v-permission:any="'manage_school|affectation_niveau_matiere.update'"
                                :icon="icons.mdiPencil" color="orange"></v-icon>
                            <v-icon end size="small" class="me-2" title="Supprimer" @click="deleteItem(tag)"
