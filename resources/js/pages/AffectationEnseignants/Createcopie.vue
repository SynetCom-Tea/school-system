    ...
    <v-card-text>
        <v-form ref="form">
            <v-row>
                ...
                <v-col cols="4" md="4">
                    <Autocomplete v-model="form.enseignant" isRequired itemValue="id" itemTitle="NomComplet" placeholder="Enseignant" label="Enseignant" chips :items="enseignants" :rules="[(v) => !!v || 'Ce champ est requis!']">
                    </Autocomplete>
                </v-col>
            </v-row>
                ...
                <v-card class="mx-auto" max-width="800">
                    <v-card-text>
                        <v-row :key="donnee.id" v-for="(donnee, i) in form.donnees">
                            <v-col md="4">
                                <Autocomplete v-model="form.niveau_matiere" isRequired itemValue="id" itemTitle="code" placeholder="Matière" label="Matière" chips :items="matieres" :rules="[(v) => !!v || 'Ce champ est requis!']">
                                </Autocomplete>
                            </v-col>
                            <v-col md="4">
                                <Autocomplete v-model="form.classe" isRequired itemValue="id" itemTitle="classe.libelle" placeholder="Classes" label="Classes" multiple chips :items="classes" :rules="[(v) => !!v || 'Ce champ est requis!']">
                                </Autocomplete>
                            </v-col>
