...
<template>
    <Toolbar styleToolbar="background-color: white;" :icon="icon.mdiSchool" :toolbarTitle="Title"></Toolbar>
    <br>
        ...
        <div style="margin: 10px">
            <v-alert v-model="alertFirst" border="start" variant="tonal" closable close-label="Close Alert"
                ...
                <li v-if="section_id <= 2">
                    Cette section vous permet de créer les classes de façon automatique en renseignant les nombres et la
                    notation souhaiter par niveau dans cet
                    établissement
                </li>
                <li v-if="section_id >= 3">
                    Cette section vous permet de créer les Niveaux de façon automatique en renseignant les nombres
                    souhaiter dans cet
                    établissement
                </li>
                <li>
            ...
            <div v-if="!alertFirst" style="margin: auto; width: 50%; padding: 10px">
                <Button size="large" style="height: 30px" title="Plier la note" @click="onclickAlertButton('first')"
                ...
                <v-form ref="form">
                    ...
                    <v-row :key="donnee.id" v-for="(donnee, i) in form.donnees">
                        ...
                        <v-col cols="4" md="4" style="height: 90px" v-if="section_id >= 3">
                            <Autocomplete label="Cycle/Filière" :items="filieres" placeholder="Cycle/Filière"
                                variant="outlined" item-value="id" class="mt-2" item-title="code"
                                v-model="donnee.filiere" isRequired chips
                                :rules="[(v) => !!v || 'Ce champ est requis!']" @update:modelValue=" verify(donnee)">
                                ></Autocomplete>
                        </v-col>
                        <v-col cols="3" md="3" style="height: 80px" v-if="section_id <= 2">
                            <Autocomplete label="Niveau" :items="niveaux" placeholder="Niveau" variant="outlined"
                                item-value="id" class="mt-2" item-title="libelle" v-model="donnee.niveau_id" isRequired
                        ...
                        <v-col cols="3" md="3" style="height: 80px" v-if="section_id <= 2">
                            <Autocomplete v-model="donnee.option" :isRequired="true" itemTitle="Option" class="mt-2"
                                placeholder="Option" label="Option" :items="['Alphabet', 'Numérique']" chips
                                :rules="[(v) => !!v || 'Ce champ est requis!']" @update:modelValue=" verify(donnee)">
                            </Autocomplete>
                            <!-- <text-field label="Genre" placeholder="Genre" v-model="form.sex" isRequired :rules="rules"></text-field> -->
                        </v-col>
                        <v-col cols="3" md="3" style="height: 80px" v-if="section_id <= 2">
                            <text-field type="number" label="Nonbre des classes" class="mt-2"
                                placeholder="Nonbre des classes" v-model="donnee.nombre" isRequired
                                :rules="[(v) => !!v || 'Ce champ est requis!']"
                                @update:modelValue=" verify(donnee)"></text-field>
                        </v-col>
                        <v-col cols="5" md="5" v-if="section_id >= 3">
                            <Autocomplete label="Niveau" :items="niveaux" placeholder="Niveau" variant="outlined"
                                item-value="id" class="mt-2" item-title="libelle" v-model="donnee.niveaux" multiple
                                chips isRequired :rules="[(v) => !!v || 'Ce champ est requis!']"
                                @update:modelValue=" verify(donnee)">
                                ></Autocomplete>
                        </v-col>
                        <v-col md="2">
                            <br>
                            <Button size="large" title="supprimer le niveau et ses classes" variant="outlined"
                                :disabled="!(form.donnees.length > 1)" icon @click="removeRow(donnee)" fab small
                                color="error">
                    ...
                    <v-row>
                        ...
                        <v-col md="2">
                            <Button size="large" title="ajouter un niveau" variant="outlined" icon @click="addRow()" fab
                                small color="primary">
