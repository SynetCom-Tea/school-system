...
<template>
    <v-card>
        ...
        <v-card-text>
            <v-row>
                <v-col md="6">
                    <autocomplete label="Enfants" v-model="apprenant" :items="children" class="mt-4" isRequired item-title="full_name" item-value="id">
                    </autocomplete>
                </v-col>
                <v-col md="6">
                    <autocomplete label="Type Evaluation" :disabled="!apprenant" v-model="type_evaluation" :items="filtrer" @update:modelValue="getResult()" class="mt-4" isRequired item-title="libelle" item-value="id">
                    </autocomplete>
                </v-col>

            </v-row>
            <Datatable v-if="type_evaluation != null" :displayAddButton="false" :titleDatatable="`Les notes de ${fullname}`" :headers="headers" :items="resultatsFinauxQuery">
            ...
            <br>
            <Datatable v-if="type_evaluation != null" :displayAddButton="false" :titleDatatable="`Les bulletin ${fullname}`" :headers="headersBulletin" :items="bulletinsChild">
            </Datatable>
