...
<v-card>
    ...
    <v-card-text>
        <v-toolbar flat color="white">
          ...
          ">
                <v-row v-if="sectionID == 1 || sectionID == 2">
                    ...
                    <v-col md="3">
                        <autocomplete label="Classe" v-model="form.classe" :items="classes" :disabled="!form.niveau" @update:modelValue="setData(form.classe)" class="mt-4" isRequired item-title="libelle" item-value="id"></autocomplete>
                    </v-col>
                    <v-col cols="2">
                        <autocomplete class="mt-4" v-model="form.periode" label="Periodes" itemTitle="libelle" itemValue="id" :items="periodes" variant="outlined" :isRequired="true" :disabled="!form.classe" chips clearable>
                        </autocomplete>
                    </v-col>
                    <v-col cols="3">
                    ...
                    <v-col md="1">
                        <v-btn class="mt-4" :append-icon="icons.mdiTimerSync" color="deep-purple-accent-4" @click="show()" :disabled="!form.type_evaluation">
                            Voir
                        </v-btn>
                    </v-col>
                </v-row>
                <v-row v-if="sectionID == 3">
                    <v-col md="3">
                        <autocomplete label="Filière" v-model="form.filiere" class="mt-4" :items="$page.props.filieres" @update:modelValue="setClasse(form.filiere)" item-title="code" item-value="id"></autocomplete>
                    </v-col>
                    <v-col md="4" v-if="form.filiere">
                        <autocomplete label="Classe" v-model="form.classe" :items="classes" class="mt-4" @update:modelValue="setData(form.classe)" isRequired item-title="libelle" item-value="id"></autocomplete>
                    </v-col>
                    <v-col cols="2">
                        <autocomplete class="mt-4" v-model="form.periode" label="Periodes" itemTitle="libelle" itemValue="id" :items="periodes" variant="outlined" :isRequired="true" :disabled="!form.classe" chips clearable>
                        </autocomplete>
        ...
        <v-card>
            <NoteTypeEvaluation v-if="form.type_evaluation != null" :data="formattedData" :headers="headers" :typeEvaluation="`Notes de ${ form.type_evaluation } des élèves par matière`" @open="openBulletinDialog" />
        </v-card>
        <BulletinDialog v-if="selectedApprenant !== null" v-model="dialog" :apprenant="selectedApprenant" @close="closeBulletinDialog" />
    </v-card-text>
</v-card>
</template>
