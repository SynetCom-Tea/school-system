...
<form novalidate @submit.prevent="submitForm">
    <v-container fluid>
        <v-card variant="outlined" style="border: 2px solid #7d002c">
            <v-card-title style="color: white; background-color: #7d002c">Matières</v-card-title>
            <v-divider></v-divider>
            <br />
            ...
            <v-card-text style="margin: 10px">
                <v-row>
                    <v-col>
                        <v-switch v-model="importation" @update:modelValue="submitForm(null)" color="#004980" inset :label="'Importatation d\'un fichier pour alimenter les matières'"></v-switch>
                    </v-col>
                    <v-col v-if="importation">
                        <v-file-input clearable required @change="handleFileUpload" v-model="form.fichier_matiere" @update:modelValue="submitForm(null)" label="Charger le fichier des Matières" variant="solo-inverted"></v-file-input>
                    </v-col>
                    <v-col v-if="importation">
            ...
            <v-card-text v-if="!importation">
                <v-row :key="matiere.id" v-for="(matiere, i) in form.matieres">
                    <!-- <v-col md="2"></v-col> -->
                    <v-col cols="4">
                        <TextField label="Code matiere" :isRequired="true" placeholder="Code matiere" @update:modelValue="submitForm(matiere)" v-model="matiere.code"></TextField>
                    </v-col>
                    <v-col cols="4">
                        <TextField label="Libelle matière" :isRequired="true" @update:modelValue="submitForm(matiere)" placeholder="Libelle matiere" v-model="matiere.libelle"></TextField>
                    </v-col>
                    <v-col cols="4">
                        <br />
                        ...
                        <v-tooltip v-model="tooltipModel" v-if="form.matieres.length == 1" bottom>
                            <template v-slot:activator="{ props }">
                                <Button variant="outlined" v-bind="props" icon size="large" small color="error">
                        ...
                        <Button type="button" variant="outlined" v-if="form.matieres.length >= 2" icon @click="removeRow(matiere)" size="large" small title="Supprimer la ligne" color="error">
                            <v-icon :icon="icons.mdiCloseCircle"></v-icon>
                        </Button>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col offset-md="11" cols="4">
                        <Button type="button" variant="outlined" @click="addRow" title="Ajouter une nouvelle ligne" icon size="large" color="primary">
                ...
                reader.onload = (e) => {
                    ...
                    if (sheetData.length > 0) {
                        ...
                        if (this.checkEntete(this.headers, this.contentType)) {
                            ...
                            this.$swal.fire({
                                title: "Erreur",
                                text: "L'en-tête de ce fichier ne correspond pas à celui du fichier souhaite veuillez corriger !",
                                icon: "warning",
                                confirmButtonText: "OK",
                            });
                        }
