...
<v-container fluid>
    <v-card variant="outlined" style="border: 2px solid #7d002c">
        <v-card-title style="color: white; background-color: #7d002c">Informations sur l'éléve</v-card-title>
        <v-divider></v-divider>
        <br />
        ...
        <v-form @submit.prevent="submitForm" noValidate>
            <v-card-text>
                <v-row style="margin-top: 3px;height: 80px;">
                    <!-- <v-col md="2"></v-col> -->
                    <v-col cols="4">
                        <TextField label="Nom" isRequired placeholder="Nom" @update:modelValue="submitForm()" v-model="form.nom" :rules="[(v) => !!v || 'Ce champ est requis!',(v) => /^[a-zA-Z\s]*$/.test(v) || 'Ce champ doit contenir uniquement des lettres']"></TextField>
                    </v-col>
                    <v-col cols="4">
                        <TextField label="Prénom" isRequired @update:modelValue="submitForm()" placeholder="Prénom" v-model="form.prenom" :rules="[(v) => !!v || 'Ce champ est requis!',(v) => /^[a-zA-Z\s]*$/.test(v) || 'Ce champ doit contenir uniquement des lettres']"></TextField>
                    </v-col>
                    <v-col cols="4">
                        <Autocomplete label="Sexe" isRequired @update:modelValue="submitForm()" v-model="form.sexe" :items="['Masculin', 'Féminin']" :rules="[(v) => !!v || 'Ce champ est requis!']"></Autocomplete>
                        <!-- <TextField
                label="Sexe"
                :isRequired="true"
